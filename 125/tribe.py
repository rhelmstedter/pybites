...
def get_top_books(content=None):
    ...
    # strip the white space from the text
    counts = Counter([a.text.strip() for a in soup.find_all("a", href=True) if AMAZON in a["href"]])
    return list(((book, count) for book, count in counts.items() if count >= MIN_COUNT))
