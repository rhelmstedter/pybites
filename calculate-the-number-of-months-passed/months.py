...
def calc_months_passed(year: int, month: int, day: int) -> int:
    ...
    if current_date < START_DATE:
        raise ValueError
    elif not all((isinstance(attr, int) for attr in (year, month, day))):
        raise TypeError

    r_delta = relativedelta(current_date, START_DATE)
    months = r_delta.years * MONTHS_PER_YEAR + r_delta.months
