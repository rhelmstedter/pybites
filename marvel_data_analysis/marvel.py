...
def most_popular_characters(characters=characters, top=5):
    ...
    sorted_characters = sorted(
        [c for c in characters if c.appearances],
...
def max_and_min_years_new_characters(characters=characters):
    ...
    """
    years = Counter(c.year for c in characters if c.year)
...
def get_percentage_female_characters(characters=characters):
    ...
    """
    females = len([c for c in characters if c.sex == "Female Characters"])
    total = len([c for c in characters if c.sex])
    return round((females / total) * 100, 2)


if __name__ == "__main__":
    years = max_and_min_years_new_characters()
    breakpoint()
