...
def _within_stat(val, stats, within):
    for stat in stats:
        if (stat - within <= val <= stat + within):
            return val
...
def custom_series_function(ser: pd.Series,
    ...
    """
    stats = list(ser.describe().drop(index=['count', 'std']).sort_values())
    final = ser.apply(lambda x: _within_stat(x, stats=stats, within=within)).dropna()
    print(final)
    return final
